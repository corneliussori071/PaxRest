...
async function listAvailableMeals(req: Request, supabase: any, auth: AuthContext, branchId: string) {
  ...
  let query = supabase
    .from('available_meals')
    .select('*, menu_items(name, description, image_url, media_url, media_type, base_price, calories, allergens, tags, station, preparation_time_min, menu_item_ingredients(id, name, quantity_used, unit, cost_contribution, inventory_items:ingredient_id(name)), menu_item_extras(id, name, price, is_available))', { count: 'exact' })
    .eq('branch_id', branchId);
